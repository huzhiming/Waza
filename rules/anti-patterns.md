...
| 19 | Implicit authorization escalation | User says "ok" or "looks good" about a draft, agent then executes a destructive write action (`git push`, `git tag`, `npm publish`, `gh release create`, close issue, force-push, delete branch) | Approval on a draft is not approval on the action. Destructive actions require the user to type the specific action verb in this turn ("post", "close", "push", "publish"). Approval does not carry forward to the next destructive step |
